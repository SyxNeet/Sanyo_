...
const DetailElevator = ({type = false, data, title, isJapan = false}) => {
  return (
    ...
    >
      ...
      ></div>
      <div className={`pt-[3.5rem] ${(data?.thong_so?.image.length > 1 || data?.thong_so_thang_may?.image?.length > 1) ? (type ? 'pr-0' : 'pl-0') : (type ? 'pr-[7rem]' : 'pl-[7rem]')} relative`}>
        ...
        {(data?.thong_so?.image.length > 1 || data?.thong_so_thang_may?.image?.length > 1)  && (
