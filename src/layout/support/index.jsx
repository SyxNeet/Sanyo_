...
import imgBgForm from "../../../public/images/layout/form/bgForm.png"
...
Bộ phận Kinh doanh sẽ liên hệ với bạn trong thời gian sớm nhất.
      ...
      <div className='flex flex-wrap w-[50%] max-md:w-full '>
          <div className='flex flex-col w-[48%] mr-[4%] relative max-md:mr-0 max-md:w-full'>
            <label  className={`mb-[2.88rem] font-SVNLagu text-[1rem] max-md:mb-[0.88rem] ${(windowWidth < 768 && nameFocused) && 'hidden'}  max-md:absolute max-md:top-[23%] pointer-events-none`}>Tên của bạn <span>*</span></label>
            <input type="text" placeholder={windowWidth < 768 ? '' : 'Phạm Tiến Thành'} value={name}
                onChange={handleNameChange}  onBlur={handleNameBlur} onFocus={handleNameFocus} className='border-b-2 border-[#6A6A6A] py-[0.5rem] bg-transparent border-opacity-50 outline-none placeholder:font-Iciel placeholder:text-[1rem]  placeholder:text-[#C6C8CB] placeholder:font-normal placeholder:leading-[150%] max-md:py-[0.75rem]'/>
            <span className='absolute top-full'>{nameError}</span>
          </div>
          <div className='flex flex-col w-[48%] relative max-md:w-full '>
            <label  className={`mb-[2.88rem] font-SVNLagu text-[1rem] max-md:mb-[0.88rem] ${(windowWidth < 768 && phoneFocused) && 'hidden'}  max-md:absolute max-md:top-[23%] pointer-events-none`}>Số điện thoại <span>*</span></label>
            <input type="text" placeholder={windowWidth < 768 ? '' : '123 456'} value={phone}
                onChange={handlePhoneChange}  onBlur={handlePhoneBlur} onFocus={handlePhoneFocus} className='border-b-2 border-[#6A6A6A] py-[0.5rem] border-opacity-50 bg-transparent outline-none placeholder:font-Iciel placeholder:text-[1rem] placeholder:text-[#C6C8CB] placeholder:font-normal placeholder:leading-[150%]'/>
          ...
          <div className='flex flex-col w-full mt-[3.12rem] relative max-md:mt-[1.88rem]'>
            ...
            <input type="text" placeholder={windowWidth < 768 ? '' : 'Message'} value={message}
                onChange={handleMessageChange}  onBlur={handleMessageBlur} onFocus={handleMessageFocus} className='border-b-2 border-[#6A6A6A] py-[0.5rem] border-opacity-50 bg-transparent outline-none placeholder:font-Iciel placeholder:text-[1rem] placeholder:text-[#C6C8CB] placeholder:font-normal placeholder:leading-[150%]'/>
            <span className='absolute top-full'>{messageError}</span>
          </div>
          <div className='w-full mt-[3.25rem] relative z-10'>
          <button type="submit" onClick={handleSubmit}>Gủi thông tin</button>
