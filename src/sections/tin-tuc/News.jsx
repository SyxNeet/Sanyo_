...
import DanhSachTinTucSkeleton from '@/components/danh-sach-tin-tuc/DanhSachTinTucSkeleton'
...
export default function News({
  ...
  }, [activePage, category, isMobile, isLoading, dataPosts])
  return (
    <section className='pb-[3.12rem] md:pb-[6.5rem] mt-[2rem] md:mt-[4.12rem]'>
      <div className='flex flex-col mb-5 md:flex-row md:items-center md:mb-8'>
        ...
        <Fade
          ...
          className='ml-auto'
        >
          <nav className='nav-buttons-tin-tuc-container max-md:overflow-x-auto max-md:flex max-md:flex-row max-md:flex-nowrap max-md:mt-3 max-md:-mr-3'>
            <Link
              href={`/tin-tuc`}
              className={clsx(
            ...
            <Link
              href={`/tin-tuc?category=co-hoi-nghe-nghiep`}
              className={clsx(
                'text-grey-600 border border-grey-200 rounded-full font-Iciel text-0.75 md:text-1 leading-1.5 px-[1.19rem] py-[0.56rem] md:px-[1.31rem] md:py-[0.81rem] ml-3 flex-none transition-300',
                {
                  'text-grey-900 bg-yellow-500 border-yellow-500':
                ...
                <div className='md:col-span-2 md:row-span-3 h-[15.65rem] md:h-[36.375rem]'>
                  {dataPosts && (
                    <Link
                      ...
                      className='relative block w-full h-full rounded-[0.75rem] overflow-hidden group tin-tuc-item'
                    >
                      <Image
