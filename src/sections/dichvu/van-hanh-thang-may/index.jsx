...
const Operate = ({isMobile = true, data}) => {
  ...
  return (
    <section className='mt-[8rem] max-md:mt-[3rem]'>
      <div className='flex flex-col items-center justify-center px-3 md:hidden '>
        <Fade
          direction='up'
          triggerOnce
        >
      ...
      >
        <SwiperSlide>
          <ItemOperate
            data={(data?.list_operate && data.list_operate[0]) || {}}
          />
        </SwiperSlide>
        ...
        {(Array.isArray(data?.list_operate)
          ? data.list_operate.slice(1)
          : []
        ).map((item, index) => (
