...
const ProcedureMb = ({data}) => {
  return (
    <section className='relative mb-14'>
      ...
      <h2
        className='font-SVNLagu text-[1.375rem] font-semibold leading-1.3 pt-[4.56rem] px-3 border-b border-[rgba(28,32,28,0.10)] pb-4 mb-4 [&>p>strong]:font-semibold [&>p>strong]:text-yellow-500 md:w-[75%] md:text-[4rem] md:pl-[6.25rem]'
            ...
            <SwiperSlide key={index}>
              <div className='flex flex-col md:flex-row px-[4.5rem] max-md:px-3'>
                <div className='flex flex-col w-full md:w-[50%]'>
                  ...
                  <div className='h-auto overflow-auto mb-5'>
                    {item?.list_steps?.map((item, index) => (
                      ...
                      >
                        <div className='w-[1.5rem] h-[1.5rem] rounded-[50%] bg-yellow-500 text-white font-SVNLagu text-[0.75rem] flex justify-center items-center mr-2 md:size-[3rem] md:text-[1.5rem] md:mr-4 mt-px'>
                          {index + 1}
                        </div>
                ...
                <Image
                  ...
                  width={1000}
                  height={1000}
                  className='w-full h-[22.375rem] object-cover md:w-[50%] md:h-[45rem]'
                />
              </div>
            </SwiperSlide>
          ))}
      ...
      <div className='lg:hidden flex max-md:px-3 mt-[1.5rem] justify-between px-[3rem]'>
        ...
        <div className='swiper-pagination-procedure hidden !w-[4.3125rem] rounded-full  h-[2.5rem] bg-yellow-500 bg-opacity-10 max-md:flex items-center justify-center !text-yellow-500 font-Iciel text-sm [&>span:last-child]:max-md:ml-1 [&>span:first-child]:max-md:mr-1'></div>
      </div>
    </section>
  )
}
